    ...
    def forward(self, xf, xr, num):
        ...
        for i in range(num):
            for j in range(num):
                ...
                t1 = torch.sum(ck*feat, dim=1, keepdim=True)/(torch.sum(ck, dim=1, keepdim=True)+1e-5)
                t0 = torch.sum((1-ck)*feat, dim=1, keepdim=True)/(torch.sum((1-ck), dim=1, keepdim=True)+1e-5)
